...
mkdir -p include/{common,core,distributed,api,plugins}
mkdir -p src/{common,core,distributed,api,plugins}
mkdir -p tests/{unit,integration,mocks}
mkdir -p examples/{simple_pipeline,distributed_cluster,custom_processors}
mkdir -p scripts/docker

# Verify that common/types.h exists in the correct location
if [ ! -f "include/common/types.h" ]; then
    echo -e "${GREEN}Creating include/common/types.h${NC}"
    cat > include/common/types.h << 'EOL'
